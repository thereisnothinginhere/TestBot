...
if is_module_loaded(FILENAME):
    ...
    from GPSiteInfoBot.modules.helper_funcs.chat_status import (
        connection_status,
        is_user_admin,
        user_admin)
    from GPSiteInfoBot.modules.sql import disable_sql as sql
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "GPSiteInfoBot.modules." + args[0].rsplit(".", 1)[0]
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "GPSiteInfoBot.modules." + args[0].rsplit(".", 1)[0]
