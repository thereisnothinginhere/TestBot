from GPSiteInfoBot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
from GPSiteInfoBot import SUDO_USERS
...
async def GPSiteInfoBot_is_admin(chat_id: int):
    status = False
    GPSiteInfoBot = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins):
        
        if GPSiteInfoBot.id == user.id:
            status = True
            break
    return status
